...
export default function OwnerRooms() {
    ...
    const fetchRooms = async (khachSanId) => {
        ...
        setLoading(true);
        try {
            const res = await api.get(`/chu-khach-san/khach-san/${khachSanId}/phong`);
    ...
    const columns = [
        ...
        { title: "Hành động", width: 200, render: (_, r) => (
            <div className="flex gap-2">
                <Button type="primary" onClick={() => navigate(`/owner/rooms/${r.id}`)}>Chi tiết</Button>
            </div>
        )},
    ];
    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-2xl font-bold">Quản lý Phòng (Owner)</h1>
